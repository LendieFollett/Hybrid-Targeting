...
Z.real0 = t( rmvnorm(M, mean = mu.true0, sigma = sigma.true^2 * diag(N0) ) ) ## scores for test rankers (wouldn't observe in real life)
#Z.real1 = t( rmvnorm(M, mean = mu.true1, sigma = sigma.true^2 * diag(N1) ) ) ## scores for training rankers
Z_obs1 = rmvnorm(1, mean = mu.true1, sigma = sigma.true^2 * diag(N1) ) #OBSERVED scores. e.g., consumption for 'training' sample
